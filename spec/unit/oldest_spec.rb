...
RSpec.describe Backmeup::Commands::Oldest do
  ...
  let(:oldest_backup) { File.join(backups, '2020-12-10T11:12:13+02:00') }
  ...
  context 'with one backup' do
    before { FileUtils.mkpath(oldest_backup) }

    it 'prints that backup' do
      command.execute(output: output)
      expect(output.string).to eq("#{oldest_backup}\n")
    end
  end
  ...
  context 'with many backups' do
    before do
      ...
      FileUtils.mkpath(oldest_backup)
    end
